...
export const checkGameLevel = (req, res, next) => {
    ...
    else res.status(StatusCodes.UNAUTHORIZED).json(Utils.getReasonPhrase(StatusCodes.UNAUTHORIZED, exceptionMsg.ERR_LIVELLO_INESISTENTE))
}
/**
 * 
 * @param req 
...
export const isReqUndefined = (req, res, next) => {
    if (Object.is(req.body.color, undefined)) res.status(StatusCodes.BAD_REQUEST).json(Utils.getReasonPhrase(StatusCodes.BAD_REQUEST, exceptionMsg.ERR_REQUEST_COLOR))
    else next()
}
/**
 * 
...
export const checkPlayerColor = (req, res, next) => {
    if (!Object.is(req.body.color, boardConstants.PIECE_COLOR_WHITE) && !Object.is(req.body.color, boardConstants.PIECE_COLOR_BLACK)) { res.status(StatusCodes.BAD_REQUEST).json(Utils.getReasonPhrase(StatusCodes.BAD_REQUEST, exceptionMsg.ERR_CAMPO_COLOR)) }
    else next()
}
/**
 * 
...
export const checkBoardId = async (req, res, next) => {
    ...
    } else res.status(StatusCodes.NOT_FOUND).json(Utils.getReasonPhrase(StatusCodes.NOT_FOUND, exceptionMsg.ERR_PARAM_BOARDID))
}
/**
 * 
 * @param req 
...
export const checkPieceMove = async (req, res, next) => {
    ...
    if (data.length == 0) res.status(StatusCodes.NOT_FOUND).json(Utils.getReasonPhrase(StatusCodes.NOT_FOUND, exceptionMsg.PARTITA_INESISTENTE_BY_ID))
        ...
        else res.status(StatusCodes.BAD_REQUEST).json(Utils.getReasonPhrase(StatusCodes.BAD_REQUEST, exceptionMsg.ERR_MOSSA_NON_VALIDA))
    }
}
/**
 * 
...
export const checkGameState = async (req, res, next) => {
    var data = await findGameByBoardId(req.params.boardid, Utils.decodeJwt(req.headers.authorization).userid)
    if (data[0].dataValues.state == boardConstants.STATE_ABANDONED || data[0].dataValues.config.isFinished) res.status(StatusCodes.BAD_REQUEST).json(Utils.getReasonPhrase(StatusCodes.BAD_REQUEST, exceptionMsg.PARTITA_ABBANDONATA))
    else next()
}
